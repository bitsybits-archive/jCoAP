 ...
 */
public class BasicCoapClientChannel extends BasicCoapChannel implements
		CoapClientChannel {
	...
	public void handleMessage(CoapMessage message) {
		if (message.isRequest()) {
			...
			message.getChannel().sendMessage(
					new CoapEmptyMessage(CoapPacketType.RST, message
							.getMessageID()));
			return;
		}
		...
		if (message.isEmpty() && message.getPacketType() == CoapPacketType.ACK) {
			...
			// TODO: implement a handler or listener, that informs a client when
			// a sep. resp. ack was received
			return;
		}
		...
		if (message.getPacketType() == CoapPacketType.CON) {
			...
			/* send ACK */
			this.sendMessage(new CoapEmptyMessage(CoapPacketType.ACK, message
					.getMessageID()));
		}
		...
		if (blockContext != null) {
			...
			if (!blockContext.isFinished()) {
				...
				if (lastRequest == null) {
					...
					System.err
							.println("ERROR: client channel: lastRequest == null");
				} else {
					/* create a new request for the next block */
					BasicCoapRequest request = new BasicCoapRequest(
							lastRequest.getPacketType(),
							lastRequest.getRequestCode(),
							channelManager.getNewMessageID());
					request.copyHeaderOptions((BasicCoapRequest) blockContext
							.getFirstRequest());
					...
					if (request.getRequestCode() == CoapRequestCode.GET) {
						...
						request.setPayload(blockContext
								.getNextPayload(newBlock));
					}
					sendMessage(request);
				}
				/*
				 * TODO: implement handler, inform the client that a block (but
				 * not the complete message) was received
				 */
				return;
			}
	...
	public BasicCoapRequest addBlockContext(CoapRequest request) {
		...
		if (request.getRequestCode() == CoapRequestCode.POST
				|| request.getRequestCode() == CoapRequestCode.PUT) {
			CoapBlockOption block1 = request.getBlock1();
			CoapBlockSize bSize = maxSendBlocksize;

			if (block1 != null
					&& block1.getBlockSize().getSize() < maxSendBlocksize
							.getSize())
				bSize = block1.getBlockSize();

			this.blockContext = new ClientBlockContext(bSize,
					request.getPayload());

			BasicCoapRequest firstRequest = createRequest(request.isReliable(),
			...
			System.err
					.println("ERROR: Tried to manually add BlockContext to GET request!");
	...
	private class ClientBlockContext {
		 ...
		 */
		public ClientBlockContext(CoapBlockOption blockOption,
				CoapBlockSize maxBlocksize) {
		...
		public ClientBlockContext(CoapBlockSize maxBlocksize, byte[] payload) {
			...
			this.blockNumber = 0;
			this.maxBlockNumber = this.payload.size()
					/ this.blockSize.getSize() - 1;
		...
		public boolean addBlock(CoapMessage msg, CoapBlockOption block) {
			...
			if (number * blockSize.getSize() > this.payload.size()) {
				return false;
			} else if (number * blockSize.getSize() + blockLength <= this.payload
					.size()) {
				return false;
			}
		...
		public byte[] getNextPayload(CoapBlockOption block) {
			...
			if (number == maxBlockNumber) {
				// payloadBlock = payload.substring(number * size,
				// payload.length() );
				tmp = new Byte[this.payload.size() - (number * size)];
				this.payload.subList(number * size, this.payload.size())
						.toArray(tmp);
				finished = true;
			} else {
				tmp = new Byte[block.getBlockSize().getSize()];
				this.payload.subList(number * size, number * size + size)
						.toArray(tmp);
				// payloadBlock = payload.substring(number * size, number * size
				// + size);
			}
