...
public class CoapMessageCode {
    ...
    public String toString() {
        ...
        if ((this.toInt() >= 64) && (this.toInt() < 192)) { /*
            ...
            String responseCode = result.substring(responseCodeIndex + 1, result.length());
            responseCode = responseCode.charAt(0) + "." + responseCode.charAt(1)
                    + responseCode.charAt(2);
            /* eventually, replace underscores by spaces */
            result = responseCode + ": " + result.substring(0, responseCodeIndex).replace("_", " ");
        }
        return result;
    }
}
